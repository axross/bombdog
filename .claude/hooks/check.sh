...
# on stderr so the agent addresses them before finishing. Runs Vitest via
# `npm run test:unit` and Biome via `npm run lint`.
set -uo pipefail
...
CODE_GLOB='\.(ts|tsx|js|jsx|mjs|cjs|css|json)$'
code_changed() {
