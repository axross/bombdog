...
# provisions the Node toolchain, prepares a local env file, materializes the
# opt-in quality hooks, and installs dependencies so linters and tests are
# runnable as soon as the session starts.
set -euo pipefail
...
# provision the Node toolchain via mise. the Node version is pinned in .nvmrc,
# which mise reads automatically. if Node is already present on the PATH (as in
# most cloud images), mise simply confirms the pinned version.
