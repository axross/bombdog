...
- `AskUserQuestion` needs **no settings permission** — no allow-list grants it. Its availability is a property of the **client surface**: an interactive terminal or IDE renders the UI, while many web, cloud, and CI surfaces do not (the call fails with a closed permission stream). So detect availability by whether the call succeeds, not by configuration.
