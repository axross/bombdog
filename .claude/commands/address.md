...
- **A human decision — a Phase 1 must-ask, plan approval, an ambiguous finding, or a conflict judgment call:** ask it through the dedicated question UI and use the answer inline (see [Asking the Human](#asking-the-human)). When the session is headless (no human present to answer), or when a machine event is stuck, post your state, **end the turn**, and wait at zero cost; the human resumes by sending **`/address continue`** in this same session.

- MUST poll autonomously ONLY for machine events (CI, the review workflow); never keep a session alive polling for a human.
- MUST resolve human decisions through the question UI inline (see [Asking the Human](#asking-the-human)); end the turn for `/address continue` only when headless or when a machine event is stuck, and never schedule a wake-up to re-check for human input.
...
Every human-gated **decision** in this flow — a Phase 1 must-ask, plan approval under `--review-plan`, an ambiguous review finding, or a conflict-resolution judgment call — goes through the **`AskUserQuestion`** tool, Claude Code's dedicated question UI. It renders your options as selectable choices in the chat and returns the answer inline, so the run continues in the same turn instead of ending and waiting for `/address continue`.

- MUST ask via `AskUserQuestion` as the primary channel: frame the decision as concrete options (2–4), state the **default you would otherwise take** and mark it recommended, and rely on the tool's built-in "Other" choice for anything unanticipated. Never bury a decision in prose or silently assume an answer.
- MUST fall back to a marked GitHub comment that @mentions `@axross`, then **end the turn**, only when the session is headless — a routine- or API-triggered run with no human present to answer the UI — so the question survives as a durable breadcrumb and `/address continue` resumes it.
