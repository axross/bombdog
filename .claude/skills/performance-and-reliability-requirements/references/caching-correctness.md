...
- SHOULD flag a Minor recommendation that newly-added external-fetch helpers make their timing observable (e.g., a `duration` measurement around the call), so cache misses are visible.
