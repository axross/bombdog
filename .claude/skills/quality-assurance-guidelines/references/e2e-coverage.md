...
Coverage Floor review focuses on critical-severity cases where the diff adds a new route, feature, or visually distinct surface without co-located test coverage in the test directory (e2e/).
...
- MUST flag a Major when a change adds a new user-facing journey without a corresponding row in the `e2e/scenarios.md` catalog table — an incomplete catalog inflates the percentage.
