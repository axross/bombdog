...
Dev-Server Output Inspection review focuses on major-severity cases where the diff introduces new warning-level log lines that were not in the `npm run dev` output before the change (or removes existing warnings without explanation).
...
Local Production Build describes the preferred project default: ask the author to run `npm run build` then `npm run start` and reload the affected surface when the diff touches:
