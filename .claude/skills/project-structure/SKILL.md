...
- MUST place components per the tier decision in the project's React guidelines — strictly domain-agnostic primitives under `src/components/primitives/<name>/`, domain components under `src/components/tracker/<name>/` — and non-UI modules under `src/lib/`. That skill owns the tier semantics, the primitives import rule, and the split/promotion strategy.
- MUST place reusable hook functions under `src/hooks/` as `use-<name>.ts`; when logic belongs in a hook vs `src/lib` vs the component is owned by the project's React guidelines.
