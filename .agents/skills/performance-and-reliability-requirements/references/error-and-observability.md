...
- MUST flag a Major when a new `try`/`catch` is placed inside a nested helper rather than at the root call site (the request entry point / top-level handler / server action). The project rule is "let errors propagate to the root call site".
...
This review focuses on critical-severity cases where the diff logs an error through the logger instead of reporting it. The project routes errors to the error tracker via `reportError(...)`, not to the logger.

**Guidelines:**

- MUST flag a Critical when the diff calls the logger's error level for a real error — the project routes errors through `reportError(...)`.
