...
- Scenario-coverage evidence (`npm run coverage:scenarios`): a new user-facing journey is added to `e2e/scenarios.ts` and tagged; a new `must` journey is a blocker until covered. This tracks **scenario** coverage (user journeys), not E2E line coverage.
