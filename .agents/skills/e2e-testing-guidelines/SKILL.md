...
- The `e2e/scenarios.ts` catalog and the `@scn:<id>` tagging convention
...
- MUST tag every e2e test with a `@scn:<id>` scenario tag from `e2e/scenarios.ts` (via `scn(...)`); the suite tracks **scenario** coverage (user journeys), not E2E line coverage. See [scenario-coverage.md](./references/scenario-coverage.md).
