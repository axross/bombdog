...
- MUST tag every e2e test with a `@scenario:<id>` join tag from `e2e/scenarios.ts` (via `scenario(...)`) plus matching `@area:`/`@priority:` facet tags (`area(...)`/`priority(...)`); the suite tracks **scenario** coverage (user journeys), not E2E line coverage. See [scenario-coverage.md](./references/scenario-coverage.md).
