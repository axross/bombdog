...
- MUST NOT embed an unresolved product, scope, or platform decision silently as an assumption; ask it instead (per the owning workflow's ask-and-yield step, e.g. the `/address` command's plan phase).
