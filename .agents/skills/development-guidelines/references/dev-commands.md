...
- MUST run `npm run build`, when the project has a build step, after changes affect routes, metadata, data-layer config, runtime config, dependencies, or public type signatures.
...
| `npm run test:e2e -- --update-snapshots` | Regenerates end-to-end snapshots for the local platform (flag syntax depends on Playwright). |

**Guidelines:**

- MUST run `npm run format` and `npm run lint` after code or documentation edits.
- MUST run `npm run test:e2e`, when the project has an e2e suite, after a change affects a UI output surface or e2e coverage.
